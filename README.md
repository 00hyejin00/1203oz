...
2. **Integrations** → **Supabase** 메뉴로 이동하거나, [Supabase 통합 설정 페이지](https://dashboard.clerk.com/setup/supabase)로 직접 이동
3. 설정 옵션을 선택한 후 **"Activate Supabase integration"** 클릭
4. 표시된 **Clerk domain** 복사 (예: `your-app-12.clerk.accounts.dev`)
   - 이 도메인을 다음 단계에서 사용합니다
...
5. 앞서 복사한 **Clerk domain** 입력 (예: `your-app-12.clerk.accounts.dev`)
...
- 프로젝트의 `docs/clerk-supabase-integration.md` 파일에서 상세 가이드 참고
...
- `/tasks-test`: Clerk 공식 문서 예제 기반 Tasks 테이블 테스트 (RLS 정책 테스트)
- `/instruments`: Supabase 공식 문서 예제 기반 Instruments 테이블 테스트
...
> **📚 상세 가이드**: `docs/clerk-localization.md` 파일에서 Clerk 한국어 로컬라이제이션 및 커스터마이징 방법을 확인할 수 있습니다.
...
│   ├── tasks-test/       # Tasks 테이블 테스트 페이지 (Clerk 통합 예제)
│   ├── instruments/      # Instruments 테이블 테스트 페이지 (Supabase 공식 예제)
